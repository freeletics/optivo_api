...
RSpec.describe OptivoApi::Client do
  ...
  describe '#call' do
    ...
    it 'calls the savon_client' do
      allow(savon_client).to receive(:call).with('get_all', message: { :a => 'b', 'sessionId' => 123 }).and_return(savon_client)
  ...
  describe 'caching' do
    ...
    it 'has  a cache' do
      expect(client.send(:has_cache?)).to be_truthy
    end

    it 'fetches the session_id from the cache' do
      expect(cache).to receive(:fetch).with('optivo_api_session_id', expires_in: 10.minutes, force: false).and_return 666
